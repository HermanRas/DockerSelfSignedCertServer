...
echo "make-cert.sh dev-server.local"
    ...
    docker run --rm --name servercerts -v $pwd/certs:/certs -e CA_EXPIRE=365 -e SSL_EXPIRE=365 -e SSL_KEY=/certs/$domain/$domain.key -e SSL_CERT=/certs/$domain/$domain.pem -e SSL_CSR=/certs/$domain/$domain.csr -e SSL_SUBJECT=$domain paulczar/omgwtfssl
